...
class GeneralCorpus(Dataset):
    def __init__(self, ann_file, offset_file, pretrained_model_name, tokenizer=None, seq_len=64, min_seq_len=64,
    ...
    def get_sentence(self,item):
        f = open(self.ann_file, 'r', encoding=self.encoding)
        f.seek(eval(self.line_offset[item].strip())['offset'],0)
        sentence = f.read(eval(self.line_offset[item].strip())['line_len']).strip('\n').strip('\r').strip('\n')
    ...
    def __getitem__(self, item):
        ...
        raw = self.get_sentence(item)
        # tokenize
        tokens = self.tokenizer.basic_tokenizer.tokenize(raw)

        # add more tokens if len(tokens) < min_len
        _cur = (item + 1) % len(self.line_offset)
        while len(tokens) < self.min_seq_len:
            _cur_tokens = self.tokenizer.basic_tokenizer.tokenize(self.get_sentence(_cur))
            tokens.extend(_cur_tokens)
            _cur = (_cur + 1) % len(self.line_offset)
