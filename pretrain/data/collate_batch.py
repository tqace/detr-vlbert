...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        if 'image' in self.data_names:
            images = [ibatch[0] for ibatch in batch]
        ...
        for i, ibatch in enumerate(batch):
            ...
            if 'boxes_cls_scores' in self.data_names:
                boxes_cls_scores = ibatch[self.data_names.index('boxes_cls_scores')]
                out['boxes_cls_scores'] = clip_pad_boxes(boxes_cls_scores,max_boxes,pad=0)
            ...
            batch[i] = tuple(out[data_name] for data_name in self.data_names if data_name != 'image')
            if self.append_ind:
