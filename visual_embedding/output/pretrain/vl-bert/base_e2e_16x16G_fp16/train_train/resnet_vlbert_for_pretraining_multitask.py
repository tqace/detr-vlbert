...
class ResNetVLBERTForPretrainingMultitask(Module):
    def __init__(self,args, config):
        ...
        if config.NETWORK.IMAGE_FEAT_PRECOMPUTED or (not config.NETWORK.MASK_RAW_PIXELS):
            self.object_mask_visual_embedding = nn.Embedding(1, 256)
    ...
    def get_text_visual_embdding(self,text,visual_embedding):
    ...
    def forward(self,
        ...
        detection_losses , obj_reps, visual_embedding = self.image_feature_extractor(samples=samples,
                                                boxes=boxes,
                                                box_mask=box_mask,
                                                im_info=im_info,
                                                mvrc_ops=mvrc_ops,
        ...
        loss = relationship_loss.mean() + mlm_loss_wvc.mean() + mlm_loss_aux.mean() + mvrc_loss.mean() + detection_loss.mean()*0
