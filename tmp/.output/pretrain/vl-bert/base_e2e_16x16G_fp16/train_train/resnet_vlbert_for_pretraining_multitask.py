...
from common.detr_models.detr import DETR
...
class ResNetVLBERTForPretrainingMultitask(Module):
    def __init__(self,args, config):
        ...
        pretrained_state_dict = torch.load('/home/qu_tang/.cache/torch/hub/checkpoints/detr-r50-e632da11.pth')['model']
        self.image_feature_extractor.load_state_dict({k:pretrained_state_dict[k] for k in pretrained_state_dict if  "obj_downsample" not in k },strict=False)
        self.object_linguistic_embeddings = nn.Embedding(1, config.NETWORK.VLBERT.hidden_size)
        if config.NETWORK.IMAGE_FEAT_PRECOMPUTED or (not config.NETWORK.MASK_RAW_PIXELS):
            self.object_mask_visual_embedding = nn.Embedding(1, 2304)
    ...
    def forward(self,
        ...
        detection_losses , obj_reps = self.image_feature_extractor(samples=samples,
                                                boxes=boxes,
                                                box_mask=box_mask,
                                                im_info=im_info,
                                                mvrc_ops=mvrc_ops,
                                                boxes_cls_scores=boxes_cls_scores,
        ...
        detection_loss = 5*detection_losses['loss_ce']+detection_losses['loss_bbox']+detection_losses['loss_giou']
        bbox_loss = detection_losses['loss_bbox']+detection_losses['loss_giou']
        outputs.update({
        ...
        loss = relationship_loss.mean() + mlm_loss_wvc.mean() + mlm_loss_aux.mean() + mvrc_loss.mean() + detection_loss.mean()
